...
namespace quan { namespace stm32f4{ namespace detail{
   ...
   struct system_pll_constants_impl{
  ...
  #ifdef QUAN_STM32F4_PLL_VCO_INPUT_Hz  
       static_assert( (static_cast<uint32_t>(QUAN_STM32F4_SOURCE_CLOCK_Hz) % pll_vco_in ) == 0,
      "QUAN_STM32F4_SOURCE_CLOCK_Hz must be multiple of QUAN_STM32F4_PLL_VCO_INPUT_Hz");

       static_assert( (static_cast<uint32_t>(QUAN_STM32F4_SYSCLK_Hz) % pll_vco_in) == 0,
      "QUAN_STM32F4_SYSCLK_Hz  must be multiple of QUAN_STM32F4_PLL_VCO_INPUT_Hz");
       static constexpr uint32_t pll_vco_in = static_cast<uint32_t>(QUAN_STM32F4_PLL_VCO_INPUT_Hz);
  #else
      ...
      static constexpr uint32_t pll_vco = static_cast<uint32_t>(QUAN_STM32F4_SYSCLK_Hz) * pllp;
   ...
   struct system_pll_constants : system_pll_constants_impl<2>::type{
      ...
      static constexpr uint32_t  pllm = static_cast<uint32_t>(QUAN_STM32F4_SOURCE_CLOCK_Hz) / pll_vco_in;

      static_assert( (pllm >= 2 ) && (pllm <= 63) ,"pllm out of range");

      // sanity check
      static_assert(static_cast<uint32_t>(QUAN_STM32F4_SYSCLK_Hz) == ((static_cast<uint32_t>(QUAN_STM32F4_SOURCE_CLOCK_Hz) / pllm) * plln ) / pllp ,
