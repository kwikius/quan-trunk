#ifndef QUAN_TWO_D_BEZIER_HPP_INCLUDED
#define QUAN_TWO_D_BEZIER_HPP_INCLUDED
    ...
    struct filled_bezier4{
       ...
       // value_type const inc;
        ...
        point_type at( const double & t)const
        {
            ...
            double t2 = t * t;
            double t3 = t2 * t;
            point_type aa(a.x,a.y);
            point_type bb(b.x,b.y);
            point_type cc(c.x,c.y);
            point_type dd(d.x,d.y);
            point_type temp 
                    = one_m_t3 * aa 
                    + 3 * t * one_m_t2 * bb
                    + 3 * t2 * one_m_t * cc
                    + t3 * dd;
            point_type result(temp.x,temp.y);
            return result;
        }
        /*
            fill with n points
