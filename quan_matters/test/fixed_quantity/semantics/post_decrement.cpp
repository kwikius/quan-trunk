...
/*
    test post_decrement for t1_quantity
*/
...
void post_decrement_test()
{
    ...
    QUAN_CHECK_EQUAL(R2.numeric_value(), Rold.numeric_value());

    quan::force::kip F1(987.51);
    quan::force::kip Fold = F1;
    QUAN_CHECK_EQUAL( Fold.numeric_value(), F1.numeric_value());
    quan::force::kip F2 = F1--;
    QUAN_CHECK(F2.numeric_value() == Fold.numeric_value());
