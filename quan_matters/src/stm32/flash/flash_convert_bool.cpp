...
/*
   bool quan::stm32::flash::flash_convert<bool>::set (const char* symbol_name, bool const & value)
   {
      ...
      if (validate(symbol_name,quan::stm32::flash::get_flash_typeid<bool>(),symbol_index)){
         quan::dynarray<uint8_t> bytestream {sizeof(bool), main_alloc_failed};
         if ( bytestream.good() && type_to_bytestream(&value,bytestream, sizeof(bool)) ){
   ...
   bool quan::stm32::flash::flash_convert<bool>::get(const char* symbol_name, bool & value_out)
   {
      ...
      if (validate(symbol_name,quan::stm32::flash::get_flash_typeid<bool>(),symbol_index)){
         ...
         if (bytestream.good() && symtab.read_symbol(symbol_index, bytestream)){
            return bytestream_to_type(bytestream,&value_out, sizeof(bool));
         }
      }
      return false;
    }
          ...
          bool (*pfn_check)(void* value))
   {
      ...
      return quan::stm32::flash::type_to_bytestream(&value,dest, sizeof(bool));
   }
   ...
   {
       bool value = false;
       if (!bytestream_to_type(src,&value,sizeof(bool))){
         return false;
       }
