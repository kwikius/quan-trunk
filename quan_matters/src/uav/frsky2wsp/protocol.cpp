...
FrSky2WSP::protocol<Port>::~protocol()
{
   if ( m_port != nullptr){
...
template <typename Port>
FrSky2WSP::protocol<Port>::protocol(Port* port)
...
template <typename Port>
bool FrSky2WSP::protocol<Port>::good() const
{
  return m_port->good();
}
...
*/
template <typename Port>
bool FrSky2WSP::protocol<Port>::get_frame(unsigned char(& destbuf)[9])
{
   for(;;){
...
*/
template <typename Port>
void FrSky2WSP::protocol<Port>::raw_get()
{
...
*/
template <typename Port>
void FrSky2WSP::protocol<Port>::get()
{
  if (m_port->read(m_buffer + m_buffer_index,1) == 0) {
      throw std::logic_error( "failed to read byte\n");
  }
